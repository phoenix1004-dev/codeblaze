const Satistics = () => {
  return (
    <section
      className="framer-ydfzfl"
      data-framer-name="Statistics"
      id="statistics"
      data-name="Statistics"
    >
      <div
        className="framer-j92mbs outline-none flex flex-col justify-start flex-shrink-0 opacity-100"
        style={{ transform: "prespective(1200px)" }}
        data-framer-component-type="RichTextContainer"
      >
        <h2
          className="framer-text framer-styles-preset-15mg8bp"
          data-styles-preset="IXKxaceG9"
        >
          <span
            data-text-fill="true"
            className="framer-text bg-gradient-to-r from-white to-[#70bfea] !text-[35px] sm:!text-[48px] md:!text-[64px] xl:text-[100px]"
          >
            Our statistics
          </span>
        </h2>
      </div>
      <div
        className="framer-1p3o734"
        data-framer-name="Gradient Overlay"
        data-name="Gradient Overlay"
      />
      <div
        className="framer-jnpwsh !flex-col lg:!flex-row"
        data-framer-name="Statistics"
        data-name="Statistics"
      >
        <div
          className="framer-qhahhd opacity-100 !flex-none !w-full lg:!flex-1 lg:!w-1"
          style={{ transform: "prespective(1200px" }}
          data-border="true"
          data-framer-name="Projects Completed"
          data-name="Projects Completed"
        >
          <div
            className="framer-o5cyk3 outline-none flex flex-col justify-start flex-shrink-0 transform-none"
            data-framer-component-type="RichTextContainer"
          >
            <h3
              className="framer-text framer-styles-preset-6ghdir"
              data-styles-preset="E4lBliITk"
            >
              Projects completed
            </h3>
          </div>
          <div className="ssr-variant hidden-r5chz7 hidden-5tblbe hidden-ncoutt !contents">
            <div
              className="framer-1w70u9r outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <h2 className="framer-text text-white">
                <span
                  data-text-fill="true"
                  className="framer-text bg-gradient-to-r from-white to-blue-400 text-[75px] font-medium leading-none tracking-normal"
                >
                  93+
                </span>
              </h2>
            </div>
          </div>
          <div
            className="framer-82r03i hidden-5tblbe"
            data-framer-name="Line"
            data-name="Line"
          />
          <div
            className="framer-1knc46o hidden-5tblbe outline-none flex flex-col justify-start flex-shrink-0 transform-none"
            data-framer-component-type="RichTextContainer"
          >
            <p
              className="framer-text framer-styles-preset-1otixq8"
              data-styles-preset="Z8KnRQzQq"
            >
              We've successfully completed 93 top-tier projects.
            </p>
          </div>
          <div
            className="framer-b9pcv8"
            data-framer-name="Gradient"
            data-name="Gradient"
          />
        </div>
        <div className="ssr-variant hidden-5tblbe hidden-ncoutt !contents">
          <div
            className="framer-1nseq65 opacity-100 !flex-none !w-full lg:!flex-1 lg:!w-1"
            style={{ transform: "prespective(1200px)" }}
            data-border="true"
            data-framer-name="Satisfied Customers"
            data-name="Satisfied Customers"
          >
            <div
              className="framer-vhlhk6 outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <h3
                className="framer-text framer-styles-preset-6ghdir"
                data-styles-preset="E4lBliITk"
              >
                Satisfied customers
              </h3>
            </div>
            <div className="ssr-variant hidden-r5chz7 !contents">
              <div
                className="framer-1qxsy3a outline-none flex flex-col justify-start flex-shrink-0 transform-none"
                data-framer-component-type="RichTextContainer"
              >
                <h2 className="framer-text text-white">
                  <span
                    data-text-fill="true"
                    className="framer-text bg-gradient-to-r from-white to-[#70bafa] font-[Inter Tight] text-[75px] font-medium leading-[1em] tracking-[0px]"
                  >
                    100%
                  </span>
                </h2>
              </div>
            </div>
            <div
              className="framer-oaz681 hidden-5tblbe"
              data-framer-name="Line"
              data-name="Line"
            ></div>
            <div
              className="framer-1gynqu9 hidden-5tblbe outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <p
                className="framer-text framer-styles-preset-1otixq8"
                data-styles-preset="Z8KnRQzQq"
              >
                We ensure a 100% satisfaction level for our clients.
              </p>
            </div>
            <div
              className="framer-10sjmu5"
              data-framer-name="Gradient"
              data-name="Gradient"
            ></div>
          </div>
        </div>
        <div className="ssr-variant hidden-5tblbe hidden-ncoutt !contents">
          <div
            className="framer-1d2qu70 opacity-100 !flex-none !w-full lg:!flex-1 lg:!w-1"
            style={{ transform: "prespective(1200px)" }}
            data-border="true"
            data-framer-name="Hours Saved per day"
            data-name="Hours Saved per day"
          >
            <div
              className="framer-5vs0k6 outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <h3
                className="framer-text framer-styles-preset-6ghdir"
                data-styles-preset="E4lBliITk"
              >
                Hours saved per day
              </h3>
            </div>
            <div className="ssr-variant hidden-r5chz7 !contents">
              <div
                className="framer-1fhc7aj outline-none flex flex-col justify-start flex-shrink-0 transform-none"
                data-framer-component-type="RichTextContainer"
              >
                <h2 className="framer-text text-white">
                  <span
                    data-text-fill="true"
                    className="framer-text bg-gradient-to-r from-white to-[#70B0FA] font-inter-tight text-[75px] font-medium tracking-normal leading-tight"
                  >
                    3h
                  </span>
                </h2>
              </div>
            </div>
            <div
              className="framer-1vm3dnv hidden-5tblbe"
              data-framer-name="Line"
              data-name="Line"
            ></div>
            <div
              className="framer-y2rhmn hidden-5tblbe outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <p
                className="framer-text framer-styles-preset-1otixq8"
                data-styles-preset="Z8KnRQzQq"
              >
                Our solutions save our clients an average of 3 hours of work per
                day.
              </p>
            </div>
            <div
              className="framer-bwse0"
              data-framer-name="Gradient"
              data-name="Gradient"
            ></div>
          </div>
        </div>
        <div className="ssr-variant hidden-5tblbe hidden-ncoutt !contents">
          <div
            className="framer-1v7rpl6 opacity-100 !flex-none !w-full lg:!flex-1 lg:!w-1"
            style={{ transform: "prespective(1200px)" }}
            data-border="true"
            data-framer-name="Cost Saved per Month"
            data-role="Cost Saved per Month"
          >
            <div
              className="framer-3rt3ru outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <h3
                className="framer-text framer-styles-preset-6ghdir"
                data-styles-preset="E4lBliITk"
              >
                Cost saved per month
              </h3>
            </div>
            <div className="ssr-variant hidden-r5chz7 !contents">
              <div
                className="framer-m2d1c5 outline-none flex flex-col justify-start flex-shrink-0 transform-none"
                data-framer-component-type="RichTextContainer"
              >
                <h2 className="framer-text text-white">
                  <span
                    data-text-fill="true"
                    className="framer-text bg-gradient-to-r from-white to-[#70B0FA] font-inter-tight text-[75px] font-medium tracking-normal leading-tight"
                  >
                    80k
                  </span>
                </h2>
              </div>
            </div>
            <div
              className="framer-oykecz hidden-5tblbe"
              data-framer-name="Line"
              data-role="Line"
            ></div>
            <div
              className="framer-1h0f7mm hidden-5tblbe outline-none flex flex-col justify-start flex-shrink-0 transform-none"
              data-framer-component-type="RichTextContainer"
            >
              <p
                className="framer-text framer-styles-preset-1otixq8"
                data-styles-preset="Z8KnRQzQq"
              >
                Our solutions save our clients an average of $80.000 per month.
              </p>
            </div>
            <div
              className="framer-1f5ynlr"
              data-framer-name="Gradient"
              data-role="Gradient"
            ></div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Satistics;
